...
class FinancialAnalyzer:
            ...
            if stat_col:
                ...
                if not filtered_df.empty and filtered_df[weight_field].sum() > 0:
                    weighted_avg = np.average(
                        filtered_df[stat_col], weights=filtered_df[weight_field]
                    )
    ...
    def calculate_line_utilization(
        ...
        if credit_line_field not in result_df.columns:
            credit_line_field = find_column(
                result_df, ["line_amount", "credit_line", "line_limit", "limite_credito"]
            )
            if not credit_line_field:
                return result_df

        if loan_amount_field not in result_df.columns:
            loan_amount_field = find_column(
                result_df, ["outstanding_balance", "olb", "current_balance", "loan_amount"]
            )
