...
from pipeline.utils import CircuitBreaker, RateLimiter, RetryPolicy, hash_file, utc_now
...
class UnifiedIngestion:
    ...
    def _log_event(self, event: str, status: str, phase: Optional[str] = None, source: Optional[str] = None, **details: Any) -> None:
        entry = {
            "run_id": self.run_id,
            "event": event,
            "status": status,
    ...
    def ingest_file(self, file_path: Path, archive_dir: Optional[Path] = None) -> IngestionResult:
        ...
        try:
            ...
            self._log_event("raw_read", "success", rows=len(df), checksum=checksum, file_type=suffix)
