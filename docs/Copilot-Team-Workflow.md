...
- Create (or expand) `Enterprise-README.md` to summarize: Copilot access, validation branch workflow, Azure free-tier resources used (App Service F1, ACR Basic, Storage), and the GitHub Actions runs (`ci-web.yml`, `ci-analytics.yml`, `ci-main.yml`).
...
> “I’m working on `abaco-loans-analytics`. Please review `.github/workflows/ci-web.yml`, `ci-analytics.yml`, and `ci-main.yml`, ensure the Docker image build/push steps are clearly described, and help me document the deployment workflow (ACR, App Service, SQL migrations, Swagger validation). Keep everything inside the free Azure trial, note the KPIs we monitor, and suggest automation improvements for the next sprint.”
