...
def portfolio_kpis(df: pd.DataFrame) -> Tuple[Dict[str, float], pd.DataFrame]:
    ...
    work = df.copy()
    work["ltv_ratio"] = (work["loan_amount"] / work["appraised_value"]) * 100
    ...
    metrics = {
        ...
        "average_ltv": work["ltv_ratio"].mean() if not work.empty else 0.0,
        "average_dti": work["dti_ratio"].mean() if not work.empty else 0.0,
    }
    return metrics, work
