...
def portfolio_kpis(df: pd.DataFrame) -> Tuple[Dict[str, float], pd.DataFrame]:
    ...
    average_ltv = (
        enriched["ltv_ratio"].mean() if not enriched["ltv_ratio"].empty else 0.0
    )
    average_dti = float(
...
def project_growth(
    ...
    projection = pd.DataFrame({"month": schedule, "yield": yields, "loan_volume": volumes})
    projection["month"] = projection["month"].dt.strftime("%b %Y")
    return projection
