...
const buildStructuredData = (data: LandingPageData) => ({
  ...
  makesOffer: data.metrics.map((metric) => ({
    '@type': 'Offer',
    itemOffered: {
      '@type': 'Service',
      name: metric.label,
      description: metric.helper ?? metric.value,
    },
  })),
})

const cloneFallback = (): LandingPageData => ({
...
const renderProduct = (product: Product) => (
  <div key={product.title} className={styles.card}>
    <div className={styles.cardHeader}>
      <h3>{product.title}</h3>
      {product.kicker ? <span className={styles.cardKicker}>{product.kicker}</span> : null}
    </div>
    <p>{product.detail}</p>
  </div>
)
...
export default async function Home() {
  ...
  const structuredData = buildStructuredData(landingData)
  ...
  return (
    <main id="main-content" className={styles.page}>
      ...
      <header className={styles.hero}>
        ...
        <div className={styles.metrics}>{landingData.metrics.map(renderMetric)}</div>
      </header>
      ...
      <section id="products" aria-labelledby="products-heading" className={styles.section}>
        ...
        <div className={styles.cardGrid}>{landingData.products.map(renderProduct)}</div>
      </section>
      ...
      <section id="dashboards" aria-labelledby="dashboards-heading" className={styles.section}>
        ...
        <div className={styles.dashboardGrid}>{dashboards.map(renderProduct)}</div>
      ...
      <section id="demo" aria-labelledby="playbook-heading" className={styles.section}>
        ...
        <div className={styles.timeline}>{landingData.steps.map(renderStep)}</div>
      </section>
